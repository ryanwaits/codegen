...
export function StacksProvider({ config, children }: StacksProviderProps) {
  return <StacksContext.Provider value={config}>{children}</StacksContext.Provider>
}
 ...
 */
export function StacksQueryProvider({ config, children, queryClient }: StacksQueryProviderProps) {
  const client =
    queryClient ||
    new QueryClient({
      defaultOptions: {
